...
class LabTestController extends Controller
{
    public function index()
    {
        $tests = LabTest::with('inmate')
            ...
            ->paginate(20);
        return view('staff.lab-tests.index', compact('tests'));
    }

    public function show(LabTest $labTest)
    {
        abort_unless($labTest->inmate && $labTest->inmate->institution_id === auth()->user()->institution_id, 403);
        return view('staff.lab-tests.show', compact('labTest'));
    }
    ...
    public function partialUpdate(Request $request, LabTest $labTest)
    {
        ...
        if($labTest->reviewed_at){
            return redirect()->route('staff.lab-tests.show',$labTest)->with('status','This test has been accepted by the doctor and is locked.');
        }
        ...
        return redirect()->route('staff.lab-tests.show', $labTest)->with('status','Lab test updated.');
    }
}
