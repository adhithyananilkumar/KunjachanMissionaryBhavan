                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr class="position-relative">
                            <td class="ps-4">
                                <a href="{{ route('system_admin.donation-requests.show', $r) }}" class="text-decoration-none text-dark stretched-link">
                            ...
                            <td>
                                @if($r->institution)
                                    <div class="badge bg-light text-dark border">{{ $r->institution->name }}</div>
                                @else
                            ...
                            <td class="text-end pe-4 position-relative z-2">
                                <a href="{{ route('system_admin.donation-requests.show', $r) }}" class="btn btn-sm btn-outline-primary border-0"><i class="bi bi-eye"></i></a>
                                <form method="POST" action="{{ route('system_admin.donation-requests.destroy', $r) }}" class="d-inline-block" onsubmit="return confirm('Delete this request?');">
                                    @csrf @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="6" class="text-center py-5 text-muted">No donation requests found.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
