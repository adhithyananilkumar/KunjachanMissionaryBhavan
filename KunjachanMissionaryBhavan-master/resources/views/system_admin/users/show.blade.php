<x-app-layout>
    ...
    <div class="d-flex flex-wrap gap-4 align-items-start mb-4">
        ...
        <div class="ms-auto d-flex flex-wrap gap-2">
            <a href="{{ route('system_admin.users.edit',$user) }}" class="btn btn-primary"><span class="bi bi-pencil-square me-1"></span>Edit</a>
            @if($user->role !== 'developer')
                <form method="POST" action="{{ route('system_admin.users.destroy',$user) }}" onsubmit="return confirm('Delete this user?');">@csrf @method('DELETE')
    ...
    <style>
        @media (max-width: 576px){ dl.row dt{font-weight:600} }
    </style>
    <script>/* no bug toggle for system admin; streamlined */</script>
</x-app-layout>
