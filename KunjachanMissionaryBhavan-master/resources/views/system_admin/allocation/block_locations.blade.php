<x-app-layout>
  ...
  <div class="d-flex align-items-center mb-3">
    <a class="btn btn-sm btn-outline-secondary" href="{{ route('system_admin.blocks.index') }}">Back to Blocks</a>
          ...
          <tr>
            ...
            <td class="text-end">
              @if($loc->status==='maintenance')
                <form method="POST" action="{{ route('system_admin.locations.update', $loc) }}" class="d-inline" onsubmit="return confirm('Mark this location Available?')">
                  @csrf @method('PUT')
              ...
              @else
                @php $occupiedNow = $activeAssignments->count() > 0; @endphp
                <form method="POST" action="{{ route('system_admin.locations.update', $loc) }}" class="d-inline" onsubmit="return confirm('Mark this location under Maintenance? Allocation will be disabled.')">
                  @csrf @method('PUT')
              ...
              @endif
              @if($activeAssignments->count())
                <button class="btn btn-sm btn-outline-secondary ms-2" data-bs-toggle="modal" data-bs-target="#transferModal" data-inmate="{{ $activeAssignments->first()->inmate?->admission_number }}" data-inmate-name="{{ ($activeAssignments->first()->inmate?->full_name ?? 'Inmate') . ' · Admission No : ' . ($activeAssignments->first()->inmate?->admission_number ?: '—') }}" data-institution="{{ $loc->institution_id }}">Transfer</button>
              ...
              @endif
              <form method="POST" action="{{ route('system_admin.locations.destroy', $loc) }}" class="d-inline ms-2" onsubmit="return confirm('Delete this location? This cannot be undone.')">
                @csrf @method('DELETE')
        ...
        <div class="card mb-2">
          <div class="card-body">
            ...
            <div class="mt-2 d-flex gap-2 flex-wrap">
              ...
              @endif
              @if($loc->status==='maintenance')
                <form method="POST" action="{{ route('system_admin.locations.update', $loc) }}" onsubmit="return confirm('Mark this location Available?')">
                  @csrf @method('PUT')
                  <input type="hidden" name="status" value="available">
                  <button class="btn btn-sm btn-outline-warning">Mark Available</button>
                </form>
              @else
                <form method="POST" action="{{ route('system_admin.locations.update', $loc) }}" onsubmit="return confirm('Mark this location under Maintenance? Allocation will be disabled.')">
                  @csrf @method('PUT')
                  <input type="hidden" name="status" value="maintenance">
                  <button class="btn btn-sm btn-outline-warning" @disabled($occupiedNow)>Mark Maintenance</button>
                </form>
              @endif
              <form method="POST" action="{{ route('system_admin.locations.destroy', $loc) }}" onsubmit="return confirm('Delete this location? This cannot be undone.')">
                @csrf @method('DELETE')
                <button class="btn btn-sm btn-outline-danger">Delete</button>
              </form>
            </div>
  ...
  <div class="modal fade" id="createLocation" tabindex="-1">
    <div class="modal-dialog">
      <form class="modal-content" method="POST" action="{{ route('system_admin.blocks.locations.store',$block) }}">
        @csrf
  ...
  modal.addEventListener('keydown', function(ev){ if(ev.key==='Enter'){ ev.preventDefault(); } });
      modal.addEventListener('show.bs.modal', function(e){
        ...
        form.action = `{{ url('system-admin/inmates/assign-location-by-admission') }}`;
        document.getElementById('transferLocationId').value = '';
        transferSubmit.disabled = true;
        load();
      });
      async function load(){
        const res = await fetch(`{{ url('system-admin/allocation/api/institutions') }}/${instId}/locations?show_occupied=${showOcc.checked?1:0}`, {headers:{'Accept':'application/json'}});
  ...
  allocModal.addEventListener('show.bs.modal', function(e){
        ...
        allocForm.action = `{{ url('system-admin/inmates') }}`; // will append /{id}/assign-location dynamically
      ...
      async function loadInmates(term){
        const url = `{{ url('system-admin/allocation/api/institutions') }}/${allocInstId}/inmates?term=${encodeURIComponent(term)}`;
      ...
      allocSubmit.addEventListener('click', async function(ev){
        ...
        const url = `{{ url('system-admin/inmates/assign-location-by-admission') }}`;
        try{
