<x-app-layout>
  <x-slot name="header"><h2 class="h5 mb-0">Medication Attendance (All Institutions)</h2></x-slot>
  ...
  <div class="card">
    <div class="table-responsive">
      <table class="table table-sm align-middle mb-0">
        <thead class="table-light"><tr><th>Time</th><th>Patient</th><th>Institution</th><th>Status</th><th>Logged By</th></tr></thead>
        <tbody>
          @foreach($logs as $log)
            <tr>
              <td>{{ $log->administration_time->format('Y-m-d H:i') }}</td>
              <td>{{ $log->medicalRecord?->inmate?->full_name ?? '—' }}</td>
              <td>{{ $log->medicalRecord?->inmate?->institution?->name ?? '—' }}</td>
