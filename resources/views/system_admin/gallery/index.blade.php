        ...
        <div class="col-6 col-md-4 col-lg-3 col-xl-2">
            <div class="card h-100 shadow-sm">
                ...
                <div class="card-body p-2 d-flex justify-content-between align-items-center">
                    ...
                    <form action="{{ route('system_admin.gallery.destroy', $image->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this image?');">
                        @csrf
                        @method('DELETE')
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="uploadForm" action="{{ route('system_admin.gallery.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        <label class="form-label">Select Image</label>
                        <input type="file" class="form-control" id="imageInput" accept="image/*" required>
                        <input type="hidden" name="image" id="finalImage"> <!-- This will store the blob/file? No, file inputs are read-only. We need to append to FormData via JS -->
                    </div>
...
<script>
    ...
    document.getElementById('cropAndUpload').addEventListener('click', function () {
        ...
        canvas.toBlob(function (blob) {
            ...
            // Replace the 'image' field with our cropped blob
            formData.set('image', blob, 'gallery-image.jpg');

            // Find form and submit via fetch or simple XHR to handle redirect manually or just submit a hidden form?
            // Easier: Attach blob to a Request and submit.
            // But standard form submission is hardest with Blob.
            // Best approach: Use fetch.
            
            const btn = document.getElementById('cropAndUpload');
            btn.disabled = true;
            btn.innerText = 'Uploading...';

            fetch('{{ route("system_admin.gallery.store") }}', {
                method: 'POST',
                body: formData,
                headers: {
            ...
            .then(response => {
                if (response.redirected) {
                    ...
                    return response.json(); // If error json
                }
            })
            .catch(error => {
