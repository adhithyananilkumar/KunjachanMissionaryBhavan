<x-app-layout>
  ...
  <div class="d-flex align-items-center gap-3 mb-3">
    ...
    <a href="{{ route('system_admin.doctors.index') }}" class="btn btn-outline-secondary btn-sm">Back</a>
  </div>
  ...
  <div class="tab-content">
    ...
    <div class="tab-pane fade" id="tabAssign">
      <div class="card">
        ...
        <div class="card-body">
          <form method="POST" action="{{ route('system_admin.doctors.assignments',$doctor) }}" id="assignForm">@csrf
  ...
  <form method="POST" action="{{ route('system_admin.doctors.emergency',$doctor) }}" id="emergencyForm">@csrf
          <div class="modal-body">
            <div class="mb-3">
              <label class="form-label">Patient (Inmate)</label>
              <select name="inmate_id" class="form-select" required>
                @foreach(\App\Models\Inmate::where('institution_id', $doctor->institution_id)
  ...
  // Guard emergency modal from Enter-key submits; submit only on explicit click
  ...
  emBtn?.addEventListener('click', function(){ emForm?.submit(); });
    // Assign chips filtering and count
    ...
    // Schedule calendar and list toggle
    const calendarEl = document.getElementById('calendar');
    const listEl = document.getElementById('scheduleList');
    const listUl = document.getElementById('scheduleItems');
    const calendarLoading = document.getElementById('calendarLoading');
    let calendar; let calendarRendered = false; let calendarInitTries = 0;
    function renderCalendarSafely() {
      ...
      if (typeof FullCalendar === 'undefined' || !calendarEl) {
        calendarInitTries++;
        if (calendarInitTries > 40) {
          // Fallback to list view if FC never loads
        ...
        setTimeout(renderCalendarSafely, 150);
        return;
      }
      calendar = new FullCalendar.Calendar(calendarEl, {
        initialView: 'dayGridMonth',
        events: { url: '{{ route('system_admin.doctors.feed',$doctor) }}' },
        eventDidMount(info){
    ...
    function loadList(){
      fetch('{{ route('system_admin.doctors.feed',$doctor) }}', {headers:{'Accept':'application/json'}})
        .then(r=>r.json()).then(items=>{
          ...
          if(!items.length){
            ...
            empty.textContent='No scheduled items yet.';
            listUl.appendChild(empty);
            return;
          }
          items.forEach(ev=>{
            ...
            li.innerHTML = `<span><span class="bi bi-calendar-event me-2"></span>${dt.toLocaleString()}<span class="text-muted ms-2">${ev.title}</span></span>` +
              (ev.extendedProps?.is_emergency ? '<span class="badge bg-danger">Emergency</span>' : '');
  ...
  document.getElementById('viewCalBtn')?.addEventListener('click', function(){ this.classList.add('active'); document.getElementById('viewListBtn').classList.remove('active'); calendarEl.classList.remove('d-none'); listEl.classList.add('d-none'); renderCalendarSafely(); });
    ...
    // Render calendar when Schedule tab shown; persist tab hash
  ...
  if (target === '#tabSchedule') { renderCalendarSafely(); }
    ...
    // Extra safety: when Schedule tab button is clicked, attempt render and fallback to list
    const scheduleBtn = document.querySelector('[data-bs-target="#tabSchedule"]');
    scheduleBtn?.addEventListener('click', function(){
      setTimeout(renderCalendarSafely, 50);
      setTimeout(()=>{ if(!calendarRendered){
        // show list as fallback
        document.getElementById('viewListBtn')?.click();
      } }, 800);
    });
    // If landing with Schedule already visible (server-marked active)
    const schedulePane = document.getElementById('tabSchedule');
  if (schedulePane && schedulePane.classList.contains('show')) { renderCalendarSafely(); }
    // Jump to tab from hash on load
    const initialHash = location.hash;
    if (initialHash) {
      const hashBtn = document.querySelector(`[data-bs-target="${initialHash}"]`);
    ...
    assignForm?.addEventListener('submit', async function(e){
      ...
      try {
        ...
        if(data.ok){
          ...
          if (badge) { badge.innerText = data.assigned_count; }
          if(window.toastr){ toastr.success('Assignments updated'); } else { alert('Assignments updated'); }
        } else { throw new Error('Save failed'); }
      } catch(err){ if(window.toastr){ toastr.error('Failed to save assignments'); } else { alert('Failed to save assignments'); } }
      finally { btn?.removeAttribute('disabled'); }
    });

    // AJAX emergency scheduling
    ...
    emergencyForm?.addEventListener('submit', async function(e){
      ...
      btn?.setAttribute('disabled','disabled');
      try {
        const fd = new FormData(emergencyForm);
        const res = await fetch(emergencyForm.action, { method:'POST', headers:{ 'Accept':'application/json', 'X-CSRF-TOKEN': fd.get('_token') }, body: fd });
        const data = await res.json();
        if(data.ok){
          if(window.toastr){ toastr.success('Emergency appointment scheduled'); } else { alert('Emergency appointment scheduled'); }
          const modalEl = document.getElementById('emergencyModal');
          const modal = bootstrap.Modal.getInstance(modalEl) || new bootstrap.Modal(modalEl);
          modal.hide();
          if(calendarRendered){ calendar.refetchEvents(); }
          emergencyForm.reset();
        } else { throw new Error('Schedule failed'); }
      } catch(err){ if(window.toastr){ toastr.error('Failed to schedule'); } else { alert('Failed to schedule'); } }
      finally { btn?.removeAttribute('disabled'); }
    });
  });
  </script>
